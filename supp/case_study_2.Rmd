...
In this case study, we illustrate the basic steps for evaluating the effectiveness of four environmental surrogates for four test features (ConiferBirds, DeciduousBirds, MixedwoodBirds, and GrasslandBirds) that occur in the boreal region of Canada. Specifically, we focus on the data preparation and analysis steps that are at the core of the surrogates evaluation. The examples focus on a set ecological benchmark networks that are located in one ecoregion in the boreal region (nets_rep.shp). All of the networks in the shapefile are representative of the four environmental surrogates. In the first analysis, we are interested in evaluating whether the top ranked network, based on the four surrogates, is also representative of our four test features. In the second analysis, we compare the previous approach with a complementary approach that focuses on maximizing reprensentation for the test features. All the required input data have already been generated and are located in the "data" folder. Please contact me (pierre.vernier@gmail.com) if you come across any problems trying to replicate this sample analysis.
...
We can now read the data into R. The study area consists of the ecoregion plus intersecting watersheds (FDAs). The ecoregion and FDA boundaries will be used as the reference areas in the analyses i.e., the region that the networks are compared to in terms of surrogates and test features. Networks are allowed to extend across ecoregion boundaries to enable hydrologic connectivity. The following datasets are available in the "data" folder:
...
***
The four plots show the distribution of climate moisture index (CMI), gross primary productivity (GPP), lake edge density (LED) and landcover (LCC) in the ecoregion and its intersecting watersheds. Note that the LCC map is a categorical map, but it has too many classes to properly display in the map's legend below. The sample benchmark network is also shown on each map.
...
tm4 <- tm_shape(r4) + tm_raster(palette="YlGn", style="quantile", title = "Density") +
    tm_shape(fda) + tm_borders(lwd=1, col="blue") +
    tm_shape(eco) + tm_borders(lwd=2, col="black") +
    tm_shape(net) + tm_polygons(col="legend", title="", pal="#000000", border.col="#000000", alpha=0.5, border.alpha=0.5) +
...
# Evaluating representativeness
 ...
 In a "representativeness" analysis, we want to evaluate the representativeness of the sample network by comparing the distribution of values of each surrogate and test feature within the network to its distribution within the ecoregion (reference area).
...
names(df) <- c("Surrogate/Feature","Description","Map type","Dissimilarity")
knitr::kable(df, align="llll")
```

In this example, the network we selected can be considered to be representative of the four surrogates using a threshold of 0.2 on a scale of 0-1, with lower values indicating increasing similarity. Other thresholds can be used. The KS values for the four test features are also shown in the far right column in the table. As with the surrogates, we consider test features to be representative if they have a KS values less than or equal to 0.2. In this case, we can consider that 3 of the 4 test features are also representative of the ecoregion, with MixedwoodBirds being slightly above the threshold. This example is based on a sample size of one. By repeating this for many networks, we can model the relationship between surrogates representativeness and test features representativeness to assess whether or not a relationship exists.

<br>

# Maximizing representation

In a "representation" analysis, we want to evaluate the representation of the sample network by calculating the proportion of values of each test feature within the network to its proportion with the ecoregion and intersecting watersheds (reference area).

## Step 7 - Calculate representation
***

In step 7, we calculate the representation index (RI) for the test features using the ratio of the proportion of test features in a network to the proportion of the network in the planning region:

<center>RI = (area of test feature in network / area of network) / (area of network / area of planning region)</center>

A sample network is considered to be representative if the representation index has a value that is close to or greater than 1. The table below provides the results for the four test features using same network we used previously i.e., the top network based on the four surrogates. For this example, representation is considered to be adequate for all four test features.
...
If our focuse is on maximizing representation for a particular test feature, we may want to calculate RI for all networks in the ecoregion,  not just the top network based on the surrogates. We can then select the top network for each test feature. In the table below, we compare the KS and RI values for the top network (based on the surrogates) to the RI values for the top network overall i.e., among all networks in the shapefile.
...
names(df) <- c('Feature','Density (range)','Top "surrogate" network (KS)', 'Top "surrogate" network (RI)', 'Top overall network (RI)')
knitr::kable(df, align="lcccc")
```

The results indicate that, for each test feature, the best network for that feature came from the larger set of networks, not the top network that was representative for the four surrogates. The map below shows the location of the top network (based on representation index) in red for ConiferBirds, DeciduousBirds, MixedwoodBirds and GrasslandBirds. The sample networks used in the previous steps is also shown in grey.
