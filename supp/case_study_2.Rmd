...
In the second analysis of this case study, we compare the previous approach with a complementary approach that focuses on maximizing reprensentation for the test features. To do this, we first calculate the representation index (RI) for all of the benchmark networks in the ecoregion. We then compared the RI value for the top network (based on the surrogates) to the RI values for all other networks. In the table we compare the KS and RI values for the top network (based on the surrogates) to the RI values for the top network overall i.e., among all networks in the shapefile.
...
The results indicate that, for each test species, the best network for that species came from the larger set of networks, not just the networks that are representative for the four surrogates. We also note, that representative networks were also identified for the four species amongst the set of representative networks.
