---
title: "Appendix S2. Building and filtering networks"
output:
  html_document:
    keep_md: false
    code_folding: hide
...
* The best network from each cluster was selected as the one with the lowest Euclidean distance value for the four biophysical criteria, calculated using a vector of the four surrogate dissimilarity values. 
  * Dissimilarity values were calculated by comparing the surrogates’ distribution in each network to its distribution in the ecoregion. For the three continuous surrogates, GPP, CMI and LED, we used the Kolmogorov-Smirnov (KS) statistic. For the categorical LCC surrogate we used the Bray-Curtis statistic. 
