...
birds = c('blbw','boch','brcr','btnw','cawa','cmwa','osfl','pigr','rubl','swth','wwcr','caribou','AllBirds','ForestBirds','AllWaterfowl','CavityNesters','GroundNesters','OverwaterNesters')
...
ecoregions = sort(netstats$ecoregion)
...
for (eco in sort(ecoregions)) {
    ...
    for (spp in birds) {
        ...
        # Extract spp raster
        if (spp=='caribou') {
            rspp = raster("data/caribou/caribou.tif")
        } else if (spp %in% c('AllWaterfowl','CavityNesters','GroundNesters','OverwaterNesters')) {
            rspp = raster(paste0("data/waterfowl/",spp,".tif"))
        } else {
            rspp = raster(paste0("data/songbirds/",spp,".tif"))
        }
        r1 = mask(crop(rspp, r),r)
        r1_ncells = ncell(r1[r1>-Inf & r1<Inf])
        if (spp %in% c('caribou','AllBirds','ForestBirds','AllWaterfowl','CavityNesters','GroundNesters','OverwaterNesters')) {
            r1range = r1
        } else {
...
write_csv(x, paste0("code/input/species_stats18.csv"))
