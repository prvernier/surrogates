...
for (ecoz in ecozones) {
    ...
    for (eco in ecoregions) {
        if(file.exists(paste0('output/ecoregions_by_spp/eco_',eco,'_nets_spp_ks.csv'))) {
            x = read_csv(paste0('output/ecoregions_by_spp/eco_',eco,'_nets_spp_ks.csv'))
    ...
    write_csv(xx, paste0('output/ecozones_by_spp/ecoz_',ecoz,'_nets_spp_ks.csv'))
    xx$ecozone=ecoz
    if (z==1) {
        zz = xx
    } else {
