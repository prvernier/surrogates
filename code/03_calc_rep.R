...
# eco_to_use = net_stats$ecoregion[net_stats$rep_gd10km==0] # ecoregions with no rep networks (needs top10km calculated first)
...
for (eco in ecoList) {
    ...
    v_nets = paste0(netDir,"eco_",eco,"_networks.shp")

    if (eco %in% eco_to_use & file.exists(v_nets)) {
        ...
        nets = read_sf(v_nets) %>% st_transform(crs=prj) %>% filter(top10km==1)
        if (nrow(nets)>1000) nets = sample_n(nets, rnd)
        x = nets %>% st_set_geometry(NULL) %>%
        ...
        for (spp in sppList) {
            #if (spp %in% pba_nwb_birds) {
            if (spp %in% birds) {
                #r = raster(paste0('data/songbirds_range/',spp,"_range.tif"))
        ...
        # Extract raster stack values using networks
        nets = dplyr::select(nets, network)
        v = fda_velox$extract(nets)
        
        # calculate species KS for random networks
        for (j in 1:length(sppList)) {
            cat("...",sppList[j],"\n")
            flush.console()
            # only use species that have at least 50% of their distribution in ecoregion
            #if (sppList[j] %in% pba_nwb_birds) {
            #    pct50 = test$pct[test$zone==eco & test$species==sppList[j]]
            #} else {
            #    pct50 = 100 # this is for guilds
            #}
            ...
            for (i in 1:nrow(nets)) {
                ...
                #netpct50 = round(length(x2)/length(x2_all)*100,0) # only use networks with at least 50% of spp dist
                #x[i,paste0(sppList[j],"_eco_pct")] = pct50
                #x[i,paste0(sppList[j],"_net_pct")] = netpct50
                if (length(x1)>0 & length(x2)>0) { # & pct50>=50 & netpct50>=0.50) {
                    x[i,sppList[j]] = round(ks.test(x1, x2)$statistic,3)
                } else {
                    x[i,sppList[j]] = NaN
                }
            }   
        }
        write_csv(x, paste0("output/ecoregions/",eco,"_nets_pba_spp_ks_",rnd,".csv"))
    }
}
